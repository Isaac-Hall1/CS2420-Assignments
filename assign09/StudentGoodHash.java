...
public class StudentGoodHash {
    ...
    /**
     * hash function for hashing the
     *
     * @return
     */
    public int hashCode() {
        double sumFirstName = 0;
        double sumLastName = 0;
        for(int i = 0; i < getFirstName().length(); i++){
           sumFirstName += getFirstName().charAt(i);
        }
        for(int i = 0; i < getLastName().length(); i++){
            sumLastName += getLastName().charAt(i);
        }
        // randomizes the location to a greater effect compared to the medium hash
        return (int) (getUid() + (sumFirstName / sumLastName));
    }
}
